...
$app->get('/bids', function (Request $request, Response $response, $args) {
    ...
    foreach ($bids as &$bid) {
        $bid['photos'] = json_decode($bid['photos'], true);
    }
...
$app->post('/bids', function (Request $request, Response $response, $args) {
    $response = $response->withHeader('Content-type', 'application/json');
        ...
        if (isset($files['audio_bid'])) {
            if ($files['audio_bid']->getError() === UPLOAD_ERR_OK) {
                $data['audio_bid'] = $files['audio_bid']->getClientFilename();
            } else {
        ...
        foreach ($photos as $photo) {
            if ($photo->getError() === UPLOAD_ERR_OK) {
                $data['photos'][] = $photo->getClientFilename();
            } else {
